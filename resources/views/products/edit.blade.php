...
<div class="halaman-penuh">
    <div class="header-tambah">
        <div>
            <h1>Edit Produk</h1>
            <p>Perbarui detail untuk: <strong>{{ $product->title }}</strong></p>
        </div>
    </div>
    ...
    <div class="kartu-form">
        <form action="{{ route('products.update', $product->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="grup-formulir">
                <label for="image">Gambar Produk (Kosongkan jika tidak ingin diubah)</label>
                <input 
                    type="file" 
                    id="image" 
                    name="image" 
                ...
                @if($product->image)
                    <div class="preview-gambar">
                        ...
                        <img src="{{ asset('storage/image/'.$product->image) }}" alt="Gambar Produk" class="gambar-preview">
                    </div>
                @endif
            ...
            <div class="grup-formulir">
                ...
                <select id="id_supplier" name="id_supplier" required>
                    ...
                    @foreach($suppliers as $supplier)
                        <option value="{{ $supplier->id }}" {{ old('id_supplier', $product->supplier_id) == $supplier->id ? 'selected' : '' }}>
...
.preview-gambar { margin-top: 0.5rem; }
.gambar-preview {
    margin-top: 0.3rem;
    border-radius: 10px;
    width: 180px;
